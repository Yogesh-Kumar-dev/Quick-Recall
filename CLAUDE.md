...
### Push notifications (the one server-side feature)

Everything else in the app is client-only, but the recurring "daily quote" push notification needs a server to hold device tokens and fire on a schedule. `src/lib/mongoose.ts` connects to MongoDB (`MONGODB_URI`, cached on `global` per the standard Next.js serverless pattern) backing three models (`src/models/Device.ts`, `Notification.ts`, `NotificationTemplate.ts`). Three Route Handlers make up the API surface: `POST /api/notifications/register` and `/unregister` (public, Zod-validated, upsert/soft-disable a `Device` row by `deviceId` — no auth, since a device only ever manages its own row) and `GET /api/cron/send-quote` (bearer-token-protected via `CRON_SECRET`, triggered by a GitHub Actions schedule rather than Vercel Cron since the Hobby plan caps Vercel Cron at once a day). The cron route round-robins active `NotificationTemplate`s, sends via Firebase Cloud Messaging (`src/lib/firebase-admin.ts`, service-account credentials, server-only) to every enabled `Device`, and self-heals by disabling devices whose FCM token comes back invalid. Client-side, `src/lib/firebase-client.ts` (public `NEXT_PUBLIC_FIREBASE_*` config) registers `public/firebase-messaging-sw.js` and calls `/api/notifications/register` with the resulting token.
