...
import { selectReduxToolkitNotes } from 'store/slices/redux';
...
export default function ReduxToolkitPage() {
  ...
  const DIFFICULTY_LANDING = useMemo<LandingDifficultyCard[]>(
    () => [
      { label: 'Basic', value: 'basic', count: reduxToolkitNotes.filter((c) => c.difficulty === 'basic').length, emoji: '🟢', color: 'success', blurb: 'configureStore, createSlice, action creators' },
      { label: 'Intermediate', value: 'intermediate', count: reduxToolkitNotes.filter((c) => c.difficulty === 'intermediate').length, emoji: '🟡', color: 'warning', blurb: 'Immer, extraReducers, TypeScript setup' },
      { label: 'Advanced', value: 'advanced', count: reduxToolkitNotes.filter((c) => c.difficulty === 'advanced').length, emoji: '🔴', color: 'error', blurb: 'createEntityAdapter, normalized collections' }
    ],
    [reduxToolkitNotes]
  );
