...
import { selectRtkQueryNotes } from 'store/slices/redux';
...
export default function RtkQueryPage() {
  ...
  const DIFFICULTY_LANDING = useMemo<LandingDifficultyCard[]>(
    () => [
      { label: 'Basic', value: 'basic', count: rtkQueryNotes.filter((c) => c.difficulty === 'basic').length, emoji: '🟢', color: 'success', blurb: 'createApi, fetchBaseQuery, query hooks, mutation hooks' },
      { label: 'Intermediate', value: 'intermediate', count: rtkQueryNotes.filter((c) => c.difficulty === 'intermediate').length, emoji: '🟡', color: 'warning', blurb: 'Cache tags, invalidation, transformResponse, lifecycle' },
      { label: 'Advanced', value: 'advanced', count: rtkQueryNotes.filter((c) => c.difficulty === 'advanced').length, emoji: '🔴', color: 'error', blurb: 'Optimistic updates, streaming, custom baseQuery' }
    ],
    [rtkQueryNotes]
  );
