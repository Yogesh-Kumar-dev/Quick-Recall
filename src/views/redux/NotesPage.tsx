...
import { selectReduxNotes } from 'store/slices/redux';
...
const PAGE_TITLE = '🗄️ Redux Notes';
...
export default function ReduxNotesPage() {
  ...
  const DIFFICULTY_LANDING = useMemo<LandingDifficultyCard[]>(
    () => [
      { label: 'Basic', value: 'basic', count: reduxNotes.filter((c) => c.difficulty === 'basic').length, emoji: '🟢', color: 'success', blurb: 'Store, reducers, actions, unidirectional flow' },
      { label: 'Intermediate', value: 'intermediate', count: reduxNotes.filter((c) => c.difficulty === 'intermediate').length, emoji: '🟡', color: 'warning', blurb: 'Middleware, selectors, normalized state' },
      { label: 'Advanced', value: 'advanced', count: reduxNotes.filter((c) => c.difficulty === 'advanced').length, emoji: '🔴', color: 'error', blurb: 'Performance patterns, architecture decisions' }
    ],
    [reduxNotes]
  );
  ...
  const CATEGORY_LANDING = useMemo<LandingCategoryCard[]>(() => {
    ...
    reduxNotes.forEach((c) => map.set(c.category, (map.get(c.category) ?? 0) + 1));
  ...
  const GOTCHA_COUNT = useMemo(() => reduxNotes.filter((c) => !!c.gotcha).length, [reduxNotes]);

  const difficulties: DifficultyOption[] = useMemo(
    () => DIFFICULTY_META.map((d) => ({ ...d, count: reduxNotes.filter((c) => c.difficulty === (d.value as Note['difficulty'])).length })),
    [reduxNotes]
  );

  const categories: CategoryOption[] = useMemo(() => {
    const base = difficulty === 'all' ? reduxNotes : reduxNotes.filter((c) => c.difficulty === difficulty);
  ...
  }, [reduxNotes, difficulty]);

  const filtered = useMemo(() => {
    const q = search.toLowerCase();
    return reduxNotes.filter((c) => {
  ...
  }, [reduxNotes, search, category, difficulty, showGotchaOnly]);
    ...
    return (
      <MainCard title={PAGE_TITLE} secondary={<ReduxDevToolsHint />}>
        <SectionLanding
          ...
          title="Redux Core Notes"
          description="The fundamental building blocks of Redux — store, reducers, actions, middleware, and selectors. Master these before moving on to Redux Toolkit or RTK Query."
          totalCount={reduxNotes.length}
      ...
      secondary={
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Typography variant="caption" color="text.secondary">
            {filtered.length} of {reduxNotes.length}
          </Typography>
          <ReduxDevToolsHint />
        </Box>
      }
