...
import { selectNextjsRenderingNotes } from 'store/slices/nextjs';
...
const PAGE_TITLE = '🖥️ Rendering Strategies';
...
export default function RenderingPage() {
  ...
  const DIFFICULTY_LANDING = useMemo<LandingDifficultyCard[]>(
    () => [
      { label: 'Basic', value: 'basic', count: nextjsRenderingNotes.filter((c) => c.difficulty === 'basic').length, emoji: '🟢', color: 'success', blurb: 'Static vs Dynamic, SSG, SSR, loading.tsx' },
      { label: 'Intermediate', value: 'intermediate', count: nextjsRenderingNotes.filter((c) => c.difficulty === 'intermediate').length, emoji: '🟡', color: 'warning', blurb: 'ISR, Suspense streaming, "use cache" directive' },
      { label: 'Advanced', value: 'advanced', count: nextjsRenderingNotes.filter((c) => c.difficulty === 'advanced').length, emoji: '🔴', color: 'error', blurb: 'Partial Pre-Rendering (PPR)' }
    ],
  ...
  const CATEGORY_LANDING = useMemo<LandingCategoryCard[]>(() => {
    ...
    nextjsRenderingNotes.forEach((c) => map.set(c.category, (map.get(c.category) ?? 0) + 1));
    return Array.from(map.entries()).map(([val, count]) => ({
      value: val,
      label: val.toUpperCase() === 'PPR' ? 'PPR' : val.charAt(0).toUpperCase() + val.slice(1),
      count,
      emoji: CATEGORY_EMOJI[val] ?? '📌'
    }));
  }, [nextjsRenderingNotes]);

  const GOTCHA_COUNT = useMemo(() => nextjsRenderingNotes.filter((c) => !!c.gotcha).length, [nextjsRenderingNotes]);

  const difficulties: DifficultyOption[] = useMemo(
    () =>
      DIFFICULTY_META.map((d) => ({
        ...d,
        count: nextjsRenderingNotes.filter((c) => c.difficulty === (d.value as Note['difficulty'])).length
      })),
    [nextjsRenderingNotes]
  );

  const categories: CategoryOption[] = useMemo(() => {
    const base = difficulty === 'all' ? nextjsRenderingNotes : nextjsRenderingNotes.filter((c) => c.difficulty === difficulty);
  ...
  }, [nextjsRenderingNotes, difficulty]);

  const filtered = useMemo(() => {
    const q = search.toLowerCase();
    return nextjsRenderingNotes.filter((c) => {
  ...
  }, [nextjsRenderingNotes, search, category, difficulty, showGotchaOnly]);
    ...
    return (
      <MainCard title={PAGE_TITLE} secondary={<ReduxDevToolsHint />}>
        <SectionLanding
          ...
          description="How Next.js decides when and where to render your pages. Covers static vs dynamic rendering, SSG, SSR, ISR, streaming with Suspense, the 'use cache' directive, and Partial Pre-Rendering (PPR)."
          totalCount={nextjsRenderingNotes.length}
      ...
      secondary={
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Typography variant="caption" color="text.secondary">
            {filtered.length} of {nextjsRenderingNotes.length}
          </Typography>
          <ReduxDevToolsHint />
        </Box>
      }
