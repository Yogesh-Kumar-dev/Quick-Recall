'use client';
/**
 * NESTED COMMENTS — TYPESCRIPT VERSION
 * ──────────────────────────────────────────────────────────────────────────────
...
function addReply(comments: CommentNode[], parentId: number, reply: CommentNode): CommentNode[] {
...
function Comment({ comment, onReply }: { comment: CommentNode; onReply: (parentId: number, text: string) => void }) {
  ...
  return (
    <div style={{ borderLeft: '2px solid #ccc', paddingLeft: 12, marginTop: 10 }}>
      ...
      <button
        ...
        style={{ fontSize: 12, cursor: 'pointer', border: 'none', background: 'none', color: '#1976d2', padding: 0 }}
      >
...
export default function NestedComments() {
  ...
  const handleReply = (parentId: number, text: string) => {
    const reply: CommentNode = { id: Date.now(), text, replies: [] };
