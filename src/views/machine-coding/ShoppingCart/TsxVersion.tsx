'use client';
/**
 * SHOPPING CART — PLAIN HTML VERSION
 * ──────────────────────────────────────────────────────────────────────────────
 * Zero MUI. Same useLocalStorage hook + cart logic.
 * Cart rendered as a sidebar panel (no Drawer — just a sticky div).
 */
...
// ── useLocalStorage hook (identical to MUI version) ────────────────────────────
...
export default function ShoppingCart() {
  const [cart, setCart] = useLocalStorage<CartItem[]>('mc-cart-plain', []);
  ...
  const cartQty = (id: number) => cart.find((i) => i.productId === id)?.quantity ?? 0;
                ...
                return (
                  <div key={item.productId} style={{ border: '1px solid #f0f0f0', borderRadius: 8, padding: 10, marginBottom: 8 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <p style={{ fontSize: 12, color: '#888', margin: 0 }}>
                          ₹{p.price} × {item.quantity} = ₹{p.price * item.quantity}
                        </p>
                      </div>
                      <button
                        onClick={() => removeItem(item.productId)}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#f44336', fontSize: 16 }}
                      >
                        ✕
                      </button>
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <button onClick={() => updateQty(item.productId, -1)} style={qtyBtnStyle}>
                        −
                      </button>
