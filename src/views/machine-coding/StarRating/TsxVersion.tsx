'use client';
/**
 * STAR RATING — PLAIN HTML VERSION
 * ──────────────────────────────────────────────────────────────────────────────
 * Zero MUI. Uses ★ and ☆ unicode characters as stars.
 * Same two-state pattern: rating + hoverRating.
 */
...
function StarRatingWidget({ value, onChange }: StarRatingWidgetProps) {
  const [hoverRating, setHoverRating] = useState<number>(0);
  const activeCount: number = hoverRating || value;
...
export default function StarRating() {
  ...
  const [secondRating, setSecondRating] = useState<number>(3);
  ...
  return (
    <div>
      ...
      <div style={cardStyle}>
        ...
        <StarRatingWidget value={secondRating} onChange={setSecondRating} />
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 12 }}>
          <span
            style={{
              padding: '2px 10px',
