...
const MOCK_DATA = [
  ...
  'Next.js', 'Node.js', 'Express', 'NestJS', 'Fastify',
  'TypeScript', 'JavaScript', 'Python', 'Go', 'Rust',
  'Tailwind CSS', 'Material UI', 'Styled Components', 'Sass', 'CSS Modules',
  ...
  'Git', 'GitHub Actions', 'Zustand', 'Jotai', 'Recoil'
];
  ...
  return (
    <div style={{ maxWidth: 480 }}>
      ...
      <div style={{ position: 'relative', marginBottom: 12 }}>
        <span style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: '#999' }}>
          {isTyping || isSearching ? (
            <span style={{ display: 'inline-block', width: 14, height: 14, border: '2px solid #e0e0e0', borderTop: '2px solid #1976d2', borderRadius: '50%', animation: 'mc-spin 0.8s linear infinite' }} />
          ) : '🔍'}
        </span>
        <input
          style={{ width: '100%', padding: '9px 36px 9px 34px', border: '1px solid #ccc', borderRadius: 6, fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
      ...
      <p style={{ fontSize: 12, color: '#888', margin: '0 0 8px' }}>
        {isTyping ? 'Waiting for you to stop typing…' : isSearching ? 'Searching…' : debouncedQuery ? `${results.length} result${results.length !== 1 ? 's' : ''} for "${debouncedQuery}"` : `${MOCK_DATA.length} items in dataset`}
      </p>
