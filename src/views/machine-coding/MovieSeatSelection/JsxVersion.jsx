'use client';
/**
 * MOVIE SEAT SELECTION — JAVASCRIPT VERSION
 * ──────────────────────────────────────────────────────────────────────────────
 * React without TypeScript — plain JS syntax, no type annotations.
 *
 * State model:
 *   booked   → Set of seat ids that are already taken (non-interactive)
 *   selected → Set of seat ids the user has picked (toggle on click)
...
// A few seats are pre-booked so the three states are visible immediately.
...
export default function MovieSeatSelection() {
  const [booked, setBooked] = useState(() => new Set(INITIAL_BOOKED));
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, padding: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 16, fontSize: 12, color: '#666' }}>
        {['available', 'selected', 'booked'].map((st) => (
