...
function setParentOpenedMenu(
  ...
  for (const item of items) {
    ...
    // Match plain URLs exactly; for URLs with query params, match on the path part so that
    // e.g. '/react/machine-coding?difficulty=easy' opens the parent collapse when pathname is '/react/machine-coding'
    const itemPathOnly = item.url?.split('?')[0];
    if (item.url === pathname || (item.url?.includes('?') && itemPathOnly === pathname)) {
      setSelected(menuId ?? null);
      setOpen(true);
    }
  }
